		...
		unsigned int xid_rep, char **b, int *sz)
{
	...
	if (reply_window_[clt_nonce].size() > 1 &&
			reply_window_[clt_nonce].front().xid > xid) {
		return FORGOTTEN;
	}
	...
	//only compact for NEW requests
	//IMPORTANT: compaction must keep the last consecutive xid
	//to avoid misclassify new RPC requests as FORGOTTEN ones
	//i.e. if the reply_window contains 1,2,3,5
	//xid_rep is 3 (client has seen everything <= 3)
	//compaction must produce 3,5 instead of just 5.
